...
int main()
{
    ...
    vector <vector <double>> wyniki(1, wolne); // na rozwiazania
    ...
    cin >> z;
    cout << "Rozwiazanie po " << z << "iteracjach:\n";
    for (int Z = 0; Z < z; ++Z)
    {
        vector <double> puste(n, 0);
        wyniki.push_back(puste);
        vector <double> iloczyn = L_U * wyniki[Z];
        vector <double> skladnik = wolne - iloczyn;
        wyniki[Z + 1] = DO * skladnik;
    }
